...
# edited0823 excel_name = 'fitresults.csv'
excel_name = 'fitresults_reshape_analyzed_selected.xlsx'
#
...
# # Plot 1
# # Plot 2
# plt.hist(sx_sy, color='black', bins=50)
...
# plt.show()
plt.hist(sx_sy, bins=50, range=(0,20), density=True)
plt.xlabel('sx_sy (pixel^2)')
plt.ylabel('Frequency')
plt.xlim(0,20)
plt.savefig(os.path.join(path_folder, random_string + '-Selected_Area_histogram.png'))

a = np.array(sx_sy)
a_reshape = a.reshape((len(sx_sy)))
b = np.zeros((len(sx_sy)))
ab_stack = np.stack((a_reshape, b), axis=-1)
gm = GaussianMixture(n_components=1, covariance_type='spherical').fit(ab_stack)
print(f'N = {len(sx_sy)}')
print(f'mean = {gm.means_}')
print(f'variance = {gm.covariances_}')
...
df_attrs_dict = get_data_from_excel(path_folder, sheet_names=['x', 'y'],
                                    excel_name=excel_name, axis=0)
df_analyzed_dict = get_data_from_excel(path_folder, sheet_names=get_reshape_sheet_names(),  excel_name=excel_name,
                                       axis=1)

##  select statistical attributes for clustering analysis
select_column_x = ['x_0']    #select bead number
select_column_y = ['y_0']
sheet_name_x = ['x']
sheet_name_y = ['y']
# for sheet_name in sheet_names:
...
df_select_x = df_attrs_dict[f'{sheet_name_x}'][select_column_x]
df_select_y = df_attrs_dict[f'{sheet_name_y}'][select_column_y]

position_x = df_select_x['x']['x_0']
position_y = df_select_y['y']['y_0']
